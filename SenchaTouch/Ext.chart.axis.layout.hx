...
typedef ILayout = {
	...
	@:optional
	function processData(?series:Ext.chart.series.ISeries):Void;
	@:optional
	function setAxis(?axis:Ext.chart.axis.IAxis):Void;
	@:optional
	function snapEnds(?context:Dynamic, ?min:Float, ?max:Float, ?estStepSize:Float):Void;
	@:optional
	function trimByRange(?context:Dynamic, ?out:Dynamic, ?trimMin:Float, ?trimMax:Float):Void;
};
