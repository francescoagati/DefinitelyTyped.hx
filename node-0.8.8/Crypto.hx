...
typedef Hash = {
	function update(data:Dynamic, ?input_encoding:String):Void;
...
typedef Cipher = {
	...
	function createDecipher(algorithm:String, password:Dynamic):Decipher;
...
extern class CryptoTopLevel {
	...
	static function pbkdf2(password:String, salt:String, iterations:Float, keylen:Float, callback:Error -> String -> Dynamic):Void;
	static function randomBytes(size:Float, ?callback:Error -> Buffer -> Void):Dynamic;
}
